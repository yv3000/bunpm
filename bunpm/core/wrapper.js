...
function main() {
  try {
    ...
    // ── Long-running / interactive subcommands: stdio inherit ──────────────
    // Scripts started via `run` (npm run dev → vite dev server), plus
    // `test`, `start`, `stop`, `restart`, keep the terminal attached
    // directly. Piping them would buffer output forever and make a running
    // dev server look like a hung command. No output reformatting happens
    // for these — live streams matter more than npm-styled cosmetics.
    if (mapped.bunArgs && PASSTHROUGH_SUBCOMMANDS.has(mapped.bunArgs[0])) {
      const result = spawnSync(execPath, execArgs, {
        stdio: 'inherit',
        shell: false,
        env: bunEnv,
